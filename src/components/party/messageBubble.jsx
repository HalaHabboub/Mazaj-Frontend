...
const MessageBubble = ({ message }) => {
    ...
    // Logic to determine styles based on message type
    const isUser = type === 'user';

    // Base bubble styles
    const getBubbleStyle = () => {
        switch (type) {
            case 'user': return 'bg-primary/10 border border-primary/20 text-white rounded-tr-none';
            ...
            default: return 'bg-white/5 border border-white/5 text-gray-200 rounded-tl-none'; // Standard AI
        }
    };
    ...
    return (
        ...
        ${isUser ? 'bg-primary/20 border border-primary/30' : 'bg-ai-accent/20 border border-ai-accent/50 text-ai-accent'}`}>
            ...
            <div className={`flex flex-col gap-1 max-w-[85%] ${isUser ? 'items-end' : ''}`}>
                ...
                <div className={`p-3 rounded-2xl text-sm leading-relaxed ${getBubbleStyle()}`}>
                    {/* Special Headers for AI Actions */}
                    {type === 'ai-deny' && (
                    ...
                    {/* Embedded Song Preview (Only for accepted songs) */}
                    {song && (
